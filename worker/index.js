...
async function wsResponse(request,env){
  ...
  const pair=new WebSocketPair();const [client,server]=Object.values(pair);server.accept({allowHalfOpen:true});
  let upstream=null,closed=false,heartbeat=null,opened=false;const pending=[];
  const markOpen=()=>{if(opened||!upstream)return;opened=true;try{server.send(JSON.stringify({event:'proxy-status',status:'connected'}))}catch{}while(pending.length){try{upstream.send(pending.shift())}catch{break}}heartbeat=setInterval(()=>{try{upstream.send(JSON.stringify({action:'heartbeat'}))}catch{}},10000)};
  ...
  try{
    ...
    upstream.addEventListener('open',markOpen);upstream.addEventListener('message',e=>{try{server.send(e.data)}catch{close()}});upstream.addEventListener('error',()=>{try{server.send(JSON.stringify({event:'proxy-status',status:'error',message:'Upstream WebSocket error'}))}catch{}close()});upstream.addEventListener('close',close);if(upstream.readyState===WebSocket.OPEN)markOpen();
  }catch(e){try{server.send(JSON.stringify({event:'proxy-status',status:'error',message:String(e?.message||e)}))}catch{}close()}
  server.addEventListener('message',e=>{try{const msg=typeof e.data==='string'?e.data:new TextDecoder().decode(e.data);JSON.parse(msg);if(opened)upstream.send(msg);else pending.push(msg)}catch{try{server.send(JSON.stringify({event:'proxy-status',status:'error',message:'Invalid client message'}))}catch{}}});server.addEventListener('close',close);server.addEventListener('error',close);
